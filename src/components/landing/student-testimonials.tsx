...
import { PlayCircle, Quote, Award } from "lucide-react";
...
const TestimonialCard = ({ testimonial }: { testimonial: TTestimonial }) => {
  ...
  return (
    <>
      ...
      {/* Read More Modal (Compacted IDL Stars Card Style) */}
      <Dialog open={isReadMoreOpen} onOpenChange={setIsReadMoreOpen}>
        <DialogContent className="w-[92vw] sm:max-w-lg p-4 sm:p-6 rounded-2xl bg-white dark:bg-slate-950 border border-slate-200/80 dark:border-slate-800 shadow-2xl max-h-[85vh] overflow-y-auto">
          <DialogHeader className="sr-only">
            <DialogTitle>{testimonial.name} Story</DialogTitle>
            <DialogDescription>Full student story of {testimonial.name}</DialogDescription>
          </DialogHeader>
...
export function StudentTestimonials({ testimonials }: { testimonials: TTestimonial[] }) {
  ...
  // Hover handlers matching Expert Teachers section
  ...
  // Touch handlers matching Expert Teachers section
