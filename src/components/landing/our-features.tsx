              ...
              <Link href={feature.href} key={index} className="block flex-shrink-0 w-[300px] sm:w-[350px] group">
                <Card className="h-full rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col bg-card">
                  <CardContent className="p-8 flex-grow flex flex-col">
                    ...
                    <h3 className={`text-2xl font-bold mt-2 ${["100% Quality Education", "100% Complete Syllabus"].includes(feature.title) ? 'bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 bg-clip-text text-transparent' : 'text-foreground'}`}>{feature.title}</h3>
