    ...
    return (
        <div className="container mx-auto py-10 px-4 md:px-6">
            <section className="mb-16 animate-fade-in-up">
              <h1 className="text-2xl md:text-4xl font-black text-foreground tracking-tighter text-left uppercase">
                {activeTab.toUpperCase()} BOARD 2026: Syllabus, Date Sheet, Results & Mock Tests
              </h1>
              <p className="mt-4 max-w-4xl text-left text-muted-foreground text-base font-medium">
            ...
            <div className="flex justify-center gap-4 mb-12">
                <Button 
                    ...
                    className="rounded-full px-8 py-5 text-base font-bold tracking-widest shadow-lg transition-all hover:scale-105 active:scale-95"
                >
                    CBSE BOARD
                </Button>
                <Button 
            ...
            <section className="grid grid-cols-2 lg:grid-cols-4 gap-4 animate-fade-in-up" style={{animationDelay: '0.2s'}}>
                {resourceCards.map((card, index) => (
                    <Link key={index} href={card.href} className="block group">
                        <Card className={`h-full rounded-xl shadow-md border-none overflow-hidden transition-all duration-500 hover:shadow-xl hover:-translate-y-1 bg-gradient-to-br ${card.gradient}`}>
                            <CardContent className="p-5 flex flex-col items-center justify-center text-center text-foreground h-full min-h-[140px]">
                                <div className="mb-3 p-3 rounded-xl bg-white/40 backdrop-blur-md shadow-sm transition-transform duration-500 group-hover:scale-110 group-hover:rotate-2">
                                    {card.icon}
                                </div>
            ...
            <div key={activeTab}>
                <section className="mt-16 animate-fade-in-up" style={{ animationDelay: '0.4s' }}>
                    <div className="flex items-center justify-between mb-8 border-l-4 border-primary pl-4">
                        ...
                        <Button variant="ghost" className="font-bold text-primary hover:bg-primary/5 tracking-widest text-[10px] hidden sm:flex">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <Card key={index} className="rounded-2xl overflow-hidden shadow-xl hover:shadow-primary/5 transition-all duration-500 flex flex-col bg-card border-muted-foreground/10 group">
                            <div className="relative overflow-hidden aspect-[16/9]">
                                ...
                                <div className="absolute top-3 left-3">
                                    <Badge className="bg-primary/90 text-white font-bold text-[9px] tracking-widest px-2 py-0.5 rounded-md uppercase shadow-lg">ONLINE</Badge>
                                </div>
                            ...
                            <CardContent className="p-5 flex flex-col flex-grow">
                                ...
                                <div className="flex flex-wrap items-center gap-2 mb-4">
                                    <Badge variant="secondary" className="bg-muted text-muted-foreground font-bold text-[8px] tracking-widest rounded-md uppercase border-none">{course.language}</Badge>
                                    <Badge variant="outline" className="font-bold text-[8px] tracking-widest rounded-md uppercase border-muted-foreground/20">{activeTab.toUpperCase()}</Badge>
                                </div>

                                <div className="space-y-1.5 mb-4 text-[10px] font-bold text-muted-foreground uppercase tracking-tighter">
                                    <p className="flex items-center gap-2"><Users className="w-3 h-3" /> {course.target}</p>
                                    <p className="flex items-center gap-2"><Calendar className="w-3 h-3" /> Starts {course.startDate} • Ends {course.endDate}</p>
                                </div>
                
                                <div className="mt-auto pt-4 border-t border-muted-foreground/5">
                                    <div className="flex items-baseline gap-2 mb-1">
                                        <p className="text-2xl font-black text-primary tracking-tighter">₹{course.price}</p>
                                        <p className="text-xs text-muted-foreground line-through opacity-50 font-bold">₹{course.originalPrice}</p>
                                    </div>
                                    <div className="inline-flex items-center px-2 py-0.5 rounded-md bg-green-500/10 text-green-600 text-[9px] font-black uppercase tracking-tighter mb-4">
                                        -{course.discount}% Limited Offer
                                    </div>
                                    <Button className="w-full font-bold tracking-widest uppercase rounded-xl h-10 shadow-lg shadow-primary/10 active:scale-95 transition-all text-xs">ENROLL NOW</Button>
                                </div>
                            </CardContent>
                        </Card>
                        ))}
                ...
                <section className="w-full py-16 bg-[#070A52] text-white mt-20 rounded-[2.5rem] shadow-2xl relative overflow-hidden">
                    ...
                    <div className="container mx-auto px-4 md:px-6 relative z-10">
                        <div className="text-center mb-12">
                            <h2 className="text-2xl md:text-4xl font-black tracking-tighter uppercase">Explore Board Resources</h2>
                            <p className="text-white/60 mt-2 text-sm font-medium tracking-wide">Stay ahead with our latest blogs and updates</p>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 max-w-6xl mx-auto">
                        {blogLinks.map((link, index) => (
                            <Button key={index} asChild variant="ghost" className="w-full justify-between bg-white/5 backdrop-blur-xl text-white hover:bg-white/10 hover:text-white border border-white/10 rounded-xl p-4 h-auto group/blog">
                                <Link href={link.href}>
                                    <div className="flex items-center gap-3">
                                        <div className="bg-white/10 p-1.5 rounded-md group-hover/blog:bg-primary transition-colors">
                                            <CheckCircle className="h-3.5 w-3.5 text-white" />
                                        </div>
                                        <span className="text-xs font-bold text-left tracking-tight">{link.text}</span>
                                    </div>
                ...
                <section className="mt-20 mb-10">
                  <div className="max-w-4xl mx-auto text-center">
                    <h2 className="text-2xl md:text-3xl font-black tracking-tighter text-foreground mb-10 uppercase">{activeTab.toUpperCase()} Board FAQs</h2>
                    <Accordion type="single" collapsible className="w-full space-y-3">
                      {(activeTab === 'cbse' ? cbseFaqs : niosFaqs).map((faq, index) => (
                        <AccordionItem key={index} value={`item-${index}`} className="bg-muted/30 rounded-2xl border-none shadow-sm overflow-hidden">
                          <AccordionTrigger className="text-left p-5 font-bold text-foreground hover:no-underline text-sm md:text-base tracking-tight transition-all data-[state=open]:bg-primary/5">
                            {faq.question}
                          </AccordionTrigger>
                          <AccordionContent className="p-5 pt-0 text-left text-muted-foreground text-xs font-medium leading-relaxed bg-primary/5">
                           {faq.answer}
                          </AccordionContent>
                        </AccordionItem>
                      ))}
...
export default function SchoolPage() {
    return (
        <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><GraduationCap className="w-10 h-10 text-primary animate-bounce" /></div>}>
            <SchoolPageContent />
        </Suspense>
    );
}
