    ...
    try {
      if (type === 'class') {
        result = action === 'add' 
          ...
          result = action === 'add'
            ? await addTopic(collectionType, path.classId!, path.subjectKey!, path.chapterIndex!, path.partKey, name, order, pdfFile)
            : await editTopic(collectionType, path.classId!, path.subjectKey!, path.partKey, path.chapterIndex!, path.topicIndex!, name, order, pdfFile);
      } else if (type === 'sub-topic') {
          result = action === 'add'
            ? await addSubTopic(collectionType, path.classId!, path.subjectKey!, path.chapterIndex!, path.topicIndex!, path.partKey, name, order, pdfFile)
            : await editSubTopic(collectionType, path.classId!, path.subjectKey!, path.partKey, path.chapterIndex!, path.topicIndex!, path.subTopicIndex!, name, order, pdfFile);
      }
                    ...
                    {classDoc.data.subjects && Object.entries(classDoc.data.subjects).sort(([,a],[,b]) => (a.order || 99) - (b.order || 99)).map(([subjectKey, subjectData]) => (
                        <Accordion key={subjectKey} type="multiple" className="w-full"><AccordionItem value={`${classDoc.id}-${subjectKey}`}>
                          ...
                          <AccordionContent className="p-2 pl-6">
                            {subjectData.chapters?.sort((a,b) => (a.order || 99) - (b.order || 99)).map((chapter, chapIdx) => (
