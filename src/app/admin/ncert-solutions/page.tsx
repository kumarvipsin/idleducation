...
interface NcertSolutionDoc { id: string; data: ClassData; }
...
export default function AdminNcertSolutionsPage() {
  const [solutions, setSolutions] = useState<NcertSolutionDoc[]>([]);
  ...
  const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (type === 'class') {
        result = action === 'add' 
            ? await addClass(collectionType, name)
            : await editClass(collectionType, path.classId!, name, order);
      } else if (type === 'subject') {
          result = action === 'add' 
            ? await addSubject(collectionType, path.classId!, name)
            : await editSubject(collectionType, path.classId!, path.subjectKey!, name, order);
      } else if (type === 'part') {
          result = action === 'add'
            ? await addPart(collectionType, path.classId!, path.subjectKey!, name)
            : await editPart(collectionType, path.classId!, path.subjectKey!, path.partKey!, name, order);
      } else if (type === 'chapter') {
          result = action === 'add'
            ? await addChapter(collectionType, path.classId!, path.subjectKey!, path.partKey, name, pdfFile)
            : await editChapter(collectionType, path.classId!, path.subjectKey!, path.partKey, path.chapterIndex!, name, pdfFile);
      } else if (type === 'topic') {
          result = action === 'add'
            ? await addTopic(collectionType, path.classId!, path.subjectKey!, path.chapterIndex!, path.partKey, name, pdfFile)
            : await editTopic(collectionType, path.classId!, path.subjectKey!, path.partKey, path.chapterIndex!, path.topicIndex!, name, pdfFile);
      } else if (type === 'sub-topic') {
          result = action === 'add'
            ? await addSubTopic(collectionType, path.classId!, path.subjectKey!, path.chapterIndex!, path.topicIndex!, path.partKey, name, pdfFile)
            : await editSubTopic(collectionType, path.classId!, path.subjectKey!, path.partKey, path.chapterIndex!, path.topicIndex!, path.subTopicIndex!, name, pdfFile);
      }
  ...
  const handleDelete = async () => {
    ...
    try {
        switch(deleteState.type) {
            case 'class':
                result = await deleteClass(collectionType, deleteState.classId);
                break;
            case 'subject':
                result = await deleteSubject(collectionType, deleteState.path.classId, deleteState.path.subjectKey);
                break;
            case 'part':
                result = await deletePart(collectionType, deleteState.path.classId, deleteState.path.subjectKey, deleteState.path.partKey);
                break;
            case 'chapter':
                result = await deleteChapter(collectionType, deleteState.path.classId, deleteState.path.subjectKey, deleteState.path.partKey, deleteState.name);
                break;
            case 'topic':
                 result = await deleteTopic(collectionType, deleteState.path.classId, deleteState.path.subjectKey, deleteState.path.partKey, deleteState.path.chapterIndex, deleteState.name);
                break;
            case 'sub-topic':
                result = await deleteSubTopic(collectionType, deleteState.path.classId, deleteState.path.subjectKey, deleteState.path.partKey, deleteState.path.chapterIndex, deleteState.path.topicIndex, deleteState.name);
                break;
        }
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <div><CardTitle>Manage NCERT Solutions</CardTitle><CardDescription>Manage content for classes, subjects, parts, chapters, topics and sub-topics.</CardDescription></div>
          ...
          {loading ? renderSkeleton() : (
            <Accordion type="multiple" className="w-full space-y-4">
              {solutions.sort((a, b) => (a.data.order || 99) - (b.data.order || 99)).map((classDoc) => (
                        ...
                        <Accordion key={subjectKey} type="multiple" className="w-full"><AccordionItem value={`${classDoc.id}-${subjectKey}`}>
                          ...
                          <AccordionContent className="p-2 pl-6">
                            ...
                            {subjectData.parts && Object.entries(subjectData.parts).sort(([,a],[,b]) => (a.order || 99) - (b.order || 99)).map(([partKey, partData]) => (
                                <Accordion key={partKey} type="multiple" className="w-full"><AccordionItem value={`${subjectKey}-${partKey}`}>
