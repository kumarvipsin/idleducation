...
import { getCollection, addClass, editClass, deleteClass, addSubject, addPart, addChapter, addTopic, addSubTopic, deleteSubject, deletePart, deleteChapter, deleteTopic, deleteSubTopic, getSignedUrlForPdf, reorderArrayItem } from '@/app/actions';
    ...
    try {
      if (type === 'class') {
        result = action === 'add' ? await addClass(collectionType, name) : await editClass(collectionType, path.classId!, name, order);
          ...
          result = action === 'add' ? await addTopic(collectionType, path.classId!, path.subjectKey!, path.chapterIndex!, path.partKey, name, order, pdfFile, shortNotePdfFile, primumNotePdfFile) : await editTopic(collectionType, path.classId!, path.subjectKey!, path.partKey, path.chapterIndex!, path.topicIndex!, name, order, pdfFile, shortNotePdfFile, primumNotePdfFile);
        ...
        {modalState && (<DialogContent><DialogHeader><DialogTitle>{modalState.action === 'add' ? 'Add New' : 'Edit'} {modalState.type.replace('-', ' ')}</DialogTitle></DialogHeader>
          <form onSubmit={handleFormSubmit}>
            <div className="grid gap-4 py-4">
              ...
              {['chapter', 'topic', 'sub-topic'].includes(modalState.type) && (<div className="grid grid-cols-4 items-center gap-4"><Label htmlFor="pdf" className="text-right">Main PDF</Label><Input id="pdf" name="pdf" type="file" accept=".pdf" className="col-span-3"/></div>)}
              
              {modalState.type === 'chapter' && (<div className="grid grid-cols-4 items-center gap-4"><Label htmlFor="longNotePdf" className="text-right">Long Note PDF</Label><Input id="longNotePdf" name="longNotePdf" type="file" accept=".pdf" className="col-span-3"/></div>)}

              {['chapter', 'topic', 'sub-topic'].includes(modalState.type) && (
                <>
