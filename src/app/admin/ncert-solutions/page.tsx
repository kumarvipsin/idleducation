...
const ContentManager = ({ collectionType, title }: { collectionType: 'ncertSolutions' | 'importantQuestions', title: string }) => {
    ...
    try {
      if (type === 'class') {
        result = action === 'add' ? await addClass(collectionType, name) : await editClass(collectionType, path.classId!, name, order);
          ...
          result = action === 'add' ? await addTopic(collectionType, path.classId!, path.subjectKey!, path.chapterIndex!, path.partKey, formData) : await editTopic(collectionType, path.classId!, path.subjectKey!, path.partKey, path.chapterIndex!, path.topicIndex!, formData);
  ...
  const TopicPdfInputs = ({ prefix }: { prefix: string }) => (
              ...
              {modalState.type === 'topic' && (
                <Tabs defaultValue="ncert">
                  <TabsList className="grid w-full grid-cols-2">
                    <TabsTrigger value="ncert">NCERT PDFs</TabsTrigger>
                    <TabsTrigger value="notes">Notes PDFs</TabsTrigger>
                  </TabsList>
                  <TabsContent value="ncert" className="p-2 space-y-2">
                    <TopicPdfInputs prefix="pdfUrl" />
                  </TabsContent>
                  <TabsContent value="notes" className="p-2 space-y-2">
                    <TopicPdfInputs prefix="notePdfUrl" />
                  </TabsContent>
                </Tabs>
              )}
