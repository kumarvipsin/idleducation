...
interface Topic { name: string; pdfUrl_en?: string; pdfUrl_hi?: string; pdfUrl_demo?: string; pdfUrl_primum?: string; subTopics?: SubTopic[]; order: number; }
...
const ContentManager = ({ collectionType, title }: { collectionType: 'ncertSolutions' | 'importantQuestions', title: string }) => {
  ...
  const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const pdf_demo = formData.get('pdf_demo') as File | null;
    const pdf_primum = formData.get('pdf_primum') as File | null;
    ...
    try {
      if (type === 'class') {
        result = action === 'add' ? await addClass(collectionType, name) : await editClass(collectionType, path.classId!, name, order);
          ...
          result = action === 'add' ? await addTopic(collectionType, path.classId!, path.subjectKey!, path.chapterIndex!, path.partKey, name, order, pdf_en, pdf_hi, pdf_demo, pdf_primum) : await editTopic(collectionType, path.classId!, path.subjectKey!, path.partKey, path.chapterIndex!, path.topicIndex!, name, order, pdf_en, pdf_hi, pdf_demo, pdf_primum);
            ...
            <div className="grid gap-4 py-4">
              ...
              {modalState.type === 'topic' && (
                <>
                    ...
                    <div className="grid grid-cols-4 items-center gap-4"><Label htmlFor="pdf_demo" className="text-right">PDF (Demo)</Label><Input id="pdf_demo" name="pdf_demo" type="file" accept=".pdf" className="col-span-3"/></div>
                    <div className="grid grid-cols-4 items-center gap-4"><Label htmlFor="pdf_primum" className="text-right">PDF (Primum)</Label><Input id="pdf_primum" name="pdf_primum" type="file" accept=".pdf" className="col-span-3"/></div>
                </>
              )}
