    ...
    try {
      if (type === 'class') {
        result = action === 'add' ? await addClass(collectionType, name) : await editClass(collectionType, path.classId!, name, order);
          ...
          result = action === 'add' ? await addChapter(collectionType, path.classId!, path.subjectKey!, path.partKey, name, pdfFile) : await editChapter(collectionType, path.classId!, path.subjectKey!, path.partKey, path.chapterIndex!, name, pdfFile);
      } else if (type === 'topic') {
