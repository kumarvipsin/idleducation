...
interface Topic { name: string; pdfUrl_en?: string; pdfUrl_hi?: string; pdfUrl_en_demo?: string; pdfUrl_en_primum?: string; pdfUrl_hi_demo?: string; pdfUrl_hi_primum?: string; subTopics?: SubTopic[]; order: number; }
...
const ContentManager = ({ collectionType, title }: { collectionType: 'ncertSolutions' | 'importantQuestions', title: string }) => {
  ...
  const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const pdf_en = formData.get('pdf_en') as File | null;
    const pdf_hi = formData.get('pdf_hi') as File | null;
    const pdf_en_demo = formData.get('pdf_en_demo') as File | null;
    const pdf_en_primum = formData.get('pdf_en_primum') as File | null;
    const pdf_hi_demo = formData.get('pdf_hi_demo') as File | null;
    const pdf_hi_primum = formData.get('pdf_hi_primum') as File | null;
    ...
    try {
      if (type === 'class') {
        result = action === 'add' ? await addClass(collectionType, name) : await editClass(collectionType, path.classId!, name, order);
          ...
          result = action === 'add' ? await addChapter(collectionType, path.classId!, path.subjectKey!, path.partKey, name, pdfFile, longNotePdfFile, shortNotePdfFile, primumNotePdfFile) : await editChapter(collectionType, path.classId!, path.subjectKey!, path.partKey, path.chapterIndex!, name, pdfFile, longNotePdfFile, shortNotePdfFile, primumNotePdfFile);
      } else if (type === 'topic') {
          result = action === 'add' ? await addTopic(collectionType, path.classId!, path.subjectKey!, path.chapterIndex!, path.partKey, name, order, pdf_en, pdf_hi, pdf_en_demo, pdf_en_primum, pdf_hi_demo, pdf_hi_primum) : await editTopic(collectionType, path.classId!, path.subjectKey!, path.partKey, path.chapterIndex!, path.topicIndex!, name, order, pdf_en, pdf_hi, pdf_en_demo, pdf_en_primum, pdf_hi_demo, pdf_hi_primum);
          ...
          <form onSubmit={handleFormSubmit}>
            <div className="grid gap-4 py-4">
              ...
              {modalState.type === 'topic' && (
                <>
                    <div className="grid grid-cols-4 items-center gap-4"><Label htmlFor="pdf_en" className="text-right">PDF (English)</Label><Input id="pdf_en" name="pdf_en" type="file" accept=".pdf" className="col-span-3"/></div>
                    <div className="grid grid-cols-4 items-center gap-4"><Label htmlFor="pdf_hi" className="text-right">PDF (Hindi)</Label><Input id="pdf_hi" name="pdf_hi" type="file" accept=".pdf" className="col-span-3"/></div>
                    <div className="grid grid-cols-4 items-center gap-4"><Label htmlFor="pdf_en_demo" className="text-right">PDF (English Demo)</Label><Input id="pdf_en_demo" name="pdf_en_demo" type="file" accept=".pdf" className="col-span-3"/></div>
                    <div className="grid grid-cols-4 items-center gap-4"><Label htmlFor="pdf_en_primum" className="text-right">PDF (English Primum)</Label><Input id="pdf_en_primum" name="pdf_en_primum" type="file" accept=".pdf" className="col-span-3"/></div>
                    <div className="grid grid-cols-4 items-center gap-4"><Label htmlFor="pdf_hi_demo" className="text-right">PDF (Hindi Demo)</Label><Input id="pdf_hi_demo" name="pdf_hi_demo" type="file" accept=".pdf" className="col-span-3"/></div>
                    <div className="grid grid-cols-4 items-center gap-4"><Label htmlFor="pdf_hi_primum" className="text-right">PDF (Hindi Primum)</Label><Input id="pdf_hi_primum" name="pdf_hi_primum" type="file" accept=".pdf" className="col-span-3"/></div>
                </>
              )}

              {modalState.type === 'chapter' && (
                <>
                    <div className="grid grid-cols-4 items-center gap-4"><Label htmlFor="pdf" className="text-right">Main PDF</Label><Input id="pdf" name="pdf" type="file" accept=".pdf" className="col-span-3"/></div>
                    ...
                    <div className="grid grid-cols-4 items-center gap-4"><Label htmlFor="shortNotePdf" className="text-right">Short Note PDF</Label><Input id="shortNotePdf" name="shortNotePdf" type="file" accept=".pdf" className="col-span-3"/></div>
                    <div className="grid grid-cols-4 items-center gap-4"><Label htmlFor="primumNotePdf" className="text-right">Primum Note PDF</Label><Input id="primumNotePdf" name="primumNotePdf" type="file" accept=".pdf" className="col-span-3"/></div>
                </>
              )}
               {modalState.type === 'sub-topic' && (
                <>
                    <div className="grid grid-cols-4 items-center gap-4"><Label htmlFor="pdf" className="text-right">Main PDF</Label><Input id="pdf" name="pdf" type="file" accept=".pdf" className="col-span-3"/></div>
                    <div className="grid grid-cols-4 items-center gap-4"><Label htmlFor="shortNotePdf" className="text-right">Short Note PDF</Label><Input id="shortNotePdf" name="shortNotePdf" type="file" accept=".pdf" className="col-span-3"/></div>
                    <div className="grid grid-cols-4 items-center gap-4"><Label htmlFor="primumNotePdf" className="text-right">Primum Note PDF</Label><Input id="primumNotePdf" name="primumNotePdf" type="file" accept=".pdf" className="col-span-3"/></div>
