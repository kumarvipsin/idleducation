...
export default function PreviousYearQuestionsPage() {
    ...
    const [selectedExam, setSelectedExam] = useState<string | 'All'>('All');
        ...
        const fetchQuestions = async () => {
            ...
            if (result.success && result.data) {
                ...
                if (result.data.length > 0) {
                    const exams = Array.from(new Set((result.data as TPreviousYearQuestion[]).map(q => q.exam))).sort();
                    if (exams.length > 0) {
    ...
    const exams = useMemo(() => {
        ...
        const uniqueExams = Array.from(new Set(questions.map(q => q.exam))).sort();
        return ['All', ...uniqueExams];
    }, [questions]);
