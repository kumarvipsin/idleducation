...
const ncertData: { [key: string]: { en: { title: string, content: string }, hi: { title: string, content: string } } } = {
...
- **मिश्रधातु:** एक मिश्रधातु दो या दो से अधिक धातुओं, या एक धातु और एक अधातु का एक समांगी मिश्रण है।`
  ...
  'default': {
    en: {
      title: 'NCERT Solutions Not Found',
      content: 'Detailed solutions for this chapter will be available soon. Please check back later.'
    },
    hi: {
      title: 'एनसीईआरटी समाधान नहीं मिला',
      content: 'इस अध्याय के विस्तृत समाधान जल्द ही उपलब्ध होंगे। कृपया बाद में फिर से देखें।'
    }
  }
};


function NcertDetailsContent({ slug }: { slug: string }) {
  const searchParams = useSearchParams();
  const lang = searchParams.get('lang') === 'hi' ? 'hi' : 'en';
  
  const chapterNotes = ncertData[slug] || ncertData['default'];
...
export default function NcertDetailsPage({ params }: { params: { slug: string } }) {
    const slug = params.slug;
    return (
        <Suspense fallback={<div>Loading...</div>}>
            <NcertDetailsContent slug={slug} />
        </Suspense>
    )
}
